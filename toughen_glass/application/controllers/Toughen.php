...
class Toughen extends CI_Controller
{
	...
	public function get_work_order()
	{
		...
		$response = $this->Toughen_Model->get_work_order($tablearr);
	...
	public function detailview($cus_id)
	{
		if (!$this->session->userdata('US_Id')) {
			...
			$response = $this->Toughen_Model->detailview($cus_id);
			$data['customerdetail'] = $this->Toughen_Model->customerview($cus_id);
			$data['additional_details'] = $this->Toughen_Model->additional_details($cus_id);
			$data['order_number'] = $this->Toughen_Model->customerview($cus_id);
