...
$("#saveProduct").click(function(){
			var url='<?php echo base_url(); ?>/admin/Editproduct/insert';
			$.post(url, $( "#productForm" ).serialize(),function(response){
				if(response){
					$('#finished').html('Product details updated successfully');
					$('#finished').show();
				}
			},'json');
});
function getsubcat(cid){
	var url='<?php echo base_url(); ?>/admin/Editproduct/getSubCategory';
