...
class Agents_Model extends CI_Model{
	...
	public function get_agents($tablearr){
		//echo("select * from tbl_agents where AG_Status = 1");
		$view = "select * from tbl_agents where AG_Status = 1" ;
		if($tablearr['columnName']=='slno'||$tablearr['columnName']=='action'){
			$orderby =" order by AG_Id desc ";
		}
		else{
			$tablearr['columnName'] = 'AG_Id';
			$orderby =" order by AG_Id desc ";
		}
		$groupby=" group by AG_Id";
		$searchQuery = " ";
		if($tablearr['searchValue'] != ''){
			$searchQuery = " and (AG_Name like '%".$tablearr['searchValue']."%') ";
		}
		//print_r($searchQuery);
		...
		if($q->result()){
				...
				$detail ='<a href="Agents/thisagent/'.$row->AG_Id.'"><i class="fa fa fa-eye"></i></a>';
				$action = '<a href="sales_enquiries/performa_invoice/'.$row->AG_Id.'"><i class="mdi 		mdi-file-document"></i></a><a href="javascript:void(0);" onclick="return confirm_delete('.$row->AG_Id.');"><i class="fa fa-trash"></i></a><a href="sales_enquiries/edit/' . $row->AG_Id . '"><i class="fa fa-pencil"></i></a>';
				$data[] = array( 
					"Agent_Name"=>$row->AG_Name,
					"Agent_Code"=>ucwords($row->AG_Code),
					"Status"=>$status[$row->AG_Status],
					"slno"=>$slno,
					"action"=>$action,
					"detail" =>$detail
				);
	...
	public function insertagents($data){
		return $this->db->insert('tbl_agents', $data);
	}
	public function insert_agent_bank_accounts($dat){
		return $this->db->insert('tbl_agent_bank_accounts', $dat);
	}
	...
	*/
	...
	public function remove_agent($ag_id){
		return  $query = $this->db->query("update tbl_agents set AG_Status =  '2' WHERE AG_Id = '".$ag_id."'");
	}
	public function get_thisagent($agent_id){
		$agentData = $this->db->query("SELECT AG.*, AB.* FROM tbl_agents AS AG LEFT JOIN tbl_agent_bank_accounts AS AB ON AB.AG_Id = AB.AG_Id WHERE AG.AG_Id = '".$agent_id."'")->row();
		//$statusArr = array("1"=>"Active","3"=>"Suspend","2"=>"Closed");
		return $agentData;
	}
}
?>