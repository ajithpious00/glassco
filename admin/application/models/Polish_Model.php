...
class Polish_Model extends CI_Model{
	...
	public function get_polish($tablearr){
		//echo("select * from tbl_agents where AG_Status = 1");
		$view = "select * from tbl_polish_types where PO_Status = 1" ;
		if($tablearr['columnName']=='slno'||$tablearr['columnName']=='action'){
			$orderby =" order by PO_Id desc ";
		}
		else{
			$tablearr['columnName'] = 'PO_Id';
			$orderby =" order by PO_Id desc ";
		}
		$groupby=" group by PO_Id";
		$searchQuery = " ";
		if($tablearr['searchValue'] != ''){
			$searchQuery = " and (PO_Name like '%".$tablearr['searchValue']."%') ";
		}
		//print_r($searchQuery);
		...
		if($q->result()){
				...
				$detail ='<a href="sales_enquiries/detailview/'.$row->PO_Id.'"><i class="fa fa fa-eye"></i></a>';
				$action = '<a href="sales_enquiries/performa_invoice/'.$row->PO_Id.'"><i class="mdi 		mdi-file-document"></i></a><a href="javascript:void(0);" onclick="return confirm_delete('.$row->PO_Id.');"><i class="fa fa-trash"></i></a><a href="sales_enquiries/edit/' . $row->PO_Id . '"><i class="fa fa-pencil"></i></a>';
				$data[] = array( 
					"Polish_Name"=>$row->PO_Name,
					"HSN_Code"=>ucwords($row->PO_Hsn_Code),
	...
	public function insertpolish($data){
		return $this->db->insert('tbl_polish_types', $data);
	}
	...
	*/
	...
	public function remove_polish($pl_id){
		return  $query = $this->db->query("update tbl_polish_types set PO_Status =  '2' WHERE PO_Id = '".$pl_id."'");
	}
}
?>