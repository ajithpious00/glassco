...
class work_Order_Model extends CI_Model
{
	...
	{
		$view = "SELECT PD.*, SL.*, CU.* FROM tbl_product AS PD LEFT JOIN tbl_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id WHERE PD.status = 1 and SL.PD_Status = 5";
		...
		if ($q->result()) {
			foreach ($q->result() as $row) {
				$slno++;
				$status = array("1" => "Order Taken", "2" => "Accepted", "3" => "Approved", "4" => "Rejected", "5" => "Cutting Completed");
				$details = '<a href="polishing/detailview/' . $row->CU_Id . '"><i class="fa fa fa-eye"></i></a>';
				$detail = '<button onClick="complete(\''.$row->PD_Order_No.'\')"
				class="btn btn-primary btn-sm" id="'.$row->PD_Order_No.'"><i
