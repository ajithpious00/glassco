...
class History_Model extends CI_Model
{
	...
	{
		$view = "SELECT PD.*, SL.*, CU.* FROM tbl_product AS PD LEFT JOIN tbl_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id WHERE PD.status = 1 and SL.PD_Status = 12";
		...
		if ($q->result()) {
			foreach ($q->result() as $row) {
				$slno++;
				$status = array("1" => "Order Taken", "2" => "Accepted", "3" => "Approved", "4" => "Rejected", "12" => "Delivered");
				$detail = '<a href="history/detailview/' . $row->CU_Id . '"><i class="fa fa fa-eye"></i></a>';
				$data[] = array(
