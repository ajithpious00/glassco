...
class Sales_Enquiries_Model extends CI_Model{
	...
	public function get_sale_enquiries($tablearr,$us_id){
		$view = "SELECT PD.*, SL.*, CU.*, US.* FROM tbl_product AS PD LEFT JOIN tbl_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN user AS US ON SL.US_Id = US.US_Id WHERE SL.PD_Status != 6 and US.Usertype = '4' and SL.CU_Id = CU.CU_Id AND SL.US_Id = '".$us_id."'" ;
