...
class Sales_Enquiries_Model extends CI_Model{
	...
	public function get_sale_enquiries($tablearr,$us_id){
		...
		if($q->result()){
				...
				$action = '<a href="sales_enquiries/performa_invoice/'.$row->CU_Id.'"><i class="mdi 		mdi-file-document"></i></a><a href="javascript:void(0);" onclick="return confirm_delete('.$row->CU_Id.');"><i class="fa fa-trash"></i></a><a href="sales_enquiries/edit/' . $row->CU_Id . '"><i class="fa fa-pencil"></i></a>';
				$data[] = array( 
	...
	public function total_sqs($cus_id) {
		//echo("SELECT CU.CU_Id, SL.PD_Price, SUM(ROUND((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144, 3)) AS total_sqf, SUM(ROUND(((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144) / 10.764, 3)) AS total_sqm, SUM(ROUND((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144, 3)) OVER (PARTITION BY CU.CU_Id) AS sum_total_sqf, SUM(ROUND(((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144) / 10.764, 3)) OVER (PARTITION BY CU.CU_Id) AS sum_total_sqm FROM tbl_sale_products AS SL LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN tbl_product AS PD ON SL.PD_Id = PD.ID WHERE SL.CU_Id  = '".$cus_id."' GROUP BY CU.CU_Id");exit();
