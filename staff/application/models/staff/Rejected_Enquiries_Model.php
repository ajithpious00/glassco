<?php
class Rejected_Enquiries_Model extends CI_Model{
	protected $table_name;
    public function __construct()
    {
        parent::__construct();
    }
	public function get_reject_details($tablearr,$us_id){
		$view = "SELECT PD.*, SL.*, CU.*,US.* FROM tbl_product AS PD LEFT JOIN tbl_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN user AS US ON US.US_Id = SL.US_Id LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id WHERE PD.status = 1 and  SL.PD_Status = 4 AND SL.US_Id = '".$us_id."' OR US.Usertype =5 and SL.PD_Status = 4";
		...
		if($q->result()){
				...
				$slno++;
				$status = array("1"=>"Order Taked","2"=>"Accepted","3"=>"Approved","4"=>"Rejected","5"=>"Completed");
				$detail ='<a href="rejected_enquiries/detailview/'.$row->CU_Id.'"><i class="fa fa fa-eye"></i></a>';
				$action = '<a href="rejected_enquiries/performa_invoice/'.$row->CU_Id.'"><i class="mdi mdi-file-document"></i></a>
						<a href="javascript:void(0);" onclick="return confirm_delete('.$row->CU_Id.');">
							<i class="fa fa-trash"></i>
						</a>';
						'<a onclick="return confirm_delete('.$row->CU_Id.');"><i class="fa fa-trash"></i></a>';
						if ($row->PD_Status = 5) {
