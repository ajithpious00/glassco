...
class Customer_Invoice_Model extends CI_Model
{
	...
	{
		$view = "SELECT PD.*, SL.*, CU.*, US.*, CUI.CU_Gst_No AS GST 
        ...
        LEFT JOIN tbl_inv_customer AS CUI ON SL.CU_Id = CUI.CU_Id 
        LEFT JOIN user AS US ON SL.US_Id = US.US_Id 
        WHERE SL.PD_Status = 3
		OR SL.PD_Status = 5
        AND US.Usertype = '4' 
		...
		if ($q->result()) {
			foreach ($q->result() as $row) {
				...
				$detail = '<a href="customer_invoice/detailview/' . $row->CU_Id . '"><i class="fa fa fa-eye"></i></a>';
				$action = '<a href="' . ($row->GST == "" ? 'customer_invoice/b_to_c/' : 'customer_invoice/b_to_b/') . $row->CU_Id . '"><i class="mdi mdi-file-document"></i></a><a href="javascript:void(0);" onclick="return confirm_delete(' . $row->CU_Id . ');"><i class="fa fa-trash"></i></a><a href="customer_invoice/edit_invoice/' . $row->CU_Id . '"><i class="fa fa-pencil"></i></a>';
				$data[] = array(
	...
	{
		//echo("SELECT PD.*,SL.*,CU.*,PU.*,ED.*,US.US_Name,SL.PD_Quantity * SL.PD_Price AS total_price,SL.PD_Height * 10 As total_height_mm,SL.PD_Width * 10 AS total_width_mm, ROUND( (SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144,3) AS sqf, ROUND((( SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144 ) / 10.764,3) AS sqm,SL.PD_Quantity * ROUND(((SL.PD_Height * 10 * SL.PD_Width * 10 ) * 0.0393701 * 0.0393701 / 144 ) / 10.764,3) AS product_quantity_sqm,SUM(ROUND((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144,3)) OVER(PARTITION BY CU.CU_Id) AS sum_total_sqf, ROUND(SUM(((SL.PD_Height * 10 * SL.PD_Width * 10) * 0.0393701 * 0.0393701 / 144) / 10.764) OVER(), 3) AS sum_total_sqm FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_product_units AS PU ON SL.PD_Unit = PU.UN_Id LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN user AS US ON SL.US_Id = US.US_Id LEFT JOIN glass_edges AS ED ON SL.ED_Id = ED.ID WHERE SL.CU_Id = '".$cus_id."'");exit();
		return $this->db->query("SELECT
								...
								FROM
									tbl_product AS PD
								LEFT JOIN tbl_inv_sale_products AS SL
								ON
									PD.ID = SL.PD_Id
								LEFT JOIN tbl_product_units AS PU
								ON
									SL.PD_Unit = PU.UN_Id
								LEFT JOIN tbl_inv_customer AS CU
								ON
									SL.CU_Id = CU.CU_Id
	...
	public function total_sqs($cus_id)
	{
		...
		return $this->db->query("SELECT PD.*,SL.*,CU.*,PU.*,ED.*,US.US_Name,SL.PD_Quantity * SL.PD_Price AS total_price,
								...
								FROM tbl_product AS PD
								LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id
								LEFT JOIN tbl_product_units AS PU ON SL.PD_Unit = PU.UN_Id
								LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id
								LEFT JOIN user AS US ON SL.US_Id = US.US_Id
	...
	{
		$this->db->select('tbl_inv_customer.*,tbl_inv_sale_products.SP_Type');
		$this->db->from('tbl_inv_customer');
		$this->db->where('tbl_inv_customer.CU_Id', $cus_id);
		$this->db->join('tbl_inv_sale_products', 'tbl_inv_customer.CU_Id = tbl_inv_sale_products.CU_Id', 'left');
		$query = $this->db->get()->row();
		return $query;
		// return $this->db->query("SELECT PD.*, SL.*, CU.*, US.US_Name, US.Perfoma_Code FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN user AS US ON SL.US_Id = US.US_Id WHERE SL.CU_Id = '" . $cus_id . "'")->row();
	}
	public function agent_detail($cus_id)
	{
		//echo("SELECT AG.*, SL.* FROM tbl_inv_sale_products AS SL LEFT JOIN tbl_agents AS AG ON SL.AG_Id = AG.AG_Id WHERE SL.CU_Id = '".$cus_id."'");exit();
		return $this->db->query("SELECT AG.*, SL.* FROM tbl_inv_sale_products AS SL LEFT JOIN tbl_agents AS AG ON SL.AG_Id = AG.AG_Id WHERE SL.CU_Id = '" . $cus_id . "'")->row();
	}
	...
	{
		//return $this->db->query("select * from tbl_product where 1 = 1 ")->result();
	...
	{
		return $this->db->insert('tbl_inv_sale_products', $data);
	}
	public function fetchcust($id)
	{
		return $data = $this->db->select('*')
			->from('tbl_inv_customer')
			->where('CU_Id', $id)
			->get()
			->result();
	}
	...
	{
		return $this->db->insert('tbl_inv_customer', $cus);
	}
	...
	{
		$query = $this->db->query("SELECT * FROM tbl_inv_customer WHERE CU_Phone = '" . $data['CU_Phone'] . "'")->row();
		return $query;
	}
	...
	{
		return $this->db->insert('tbl_inv_additional_addings', $dat);
	}
	public function insert_additional_detail($da)
	{
		return $this->db->insert('tbl_inv_additional_add', $da);
	}
	...
	{
		return $this->db->query("SELECT PD.*, CU.*, SP.* FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SP ON PD.ID = SP.PD_Id LEFT JOIN tbl_inv_customer AS CU ON SP.CU_Id = CU.CU_Id WHERE CU.CU_Id = '" . $cus_id . "'")->result();
	}
	public function getheight($cus_id)
	{
		return $this->db->query("SELECT * from tbl_inv_sale_products where CU_Id = '" . $cus_id . "'")->result();
	}
	...
	{
		return  $query = $this->db->query("update tbl_inv_sale_products set PD_Status =  '6' WHERE CU_Id = '" . $cus_id . "'");
	}
	/*public function cm_to_mm($cus_id){
		return this->db->query("SELECT PD.*, SL.*, CU.*, US.US_Name, SL.PD_Height * 10 AS total_height_mm, SL.PD_Width * 10 AS total_width_mm FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN USER AS US ON SL.US_Id = US.US_Id  = '".$cus_id."'")->result();
	}
	public function inch_to_mm($cus_id){
		return this->db->query("SELECT PD.*, SL.*, CU.*, US.US_Name, SL.PD_Height * 25.4 AS total_height_mm, SL.PD_Width * 25.4 AS total_width_mm FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN USER AS US ON SL.US_Id = US.US_Id  = '".$cus_id."'")->result();
	}
	public function feet_to_mm($cus_id){
		return this->db->query("SELECT PD.*, SL.*, CU.*, US.US_Name, SL.PD_Height * 304.79999025 AS total_height_mm, SL.PD_Width * 304.79999025 AS total_width_mm FROM tbl_product AS PD LEFT JOIN tbl_inv_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_inv_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN USER AS US ON SL.US_Id = US.US_Id  = '".$cus_id."'")->result();
	}
	*/
	...
	{
		return $this->db->where('CU_Id', $id)
			->update('tbl_inv_customer', $data);
	}
	public function updatesp($id, $data)
	{
		return $this->db->where('SP_Id', $id)
			->update('tbl_inv_sale_products', $data);
	}
	public function additional_details($cus_id)
	{
		$this->db->select('tbl_inv_sale_products.*, tbl_inv_additional_addings.*');
		$this->db->from('tbl_inv_sale_products');
		$this->db->where('tbl_inv_sale_products.CU_Id', $cus_id);
		$this->db->join('tbl_inv_additional_addings', 'tbl_inv_sale_products.SP_Id = tbl_inv_additional_addings.SP_Id', 'left');
		$query = $this->db->get();
		$result = array();
	...
	{
		//echo("SELECT SUM( CASE WHEN ad.AU_Type IN (1, 3) THEN ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2) WHEN ad.AU_Type = 2 THEN ROUND(AD_Unit_Price * 100 / 118, 2) ELSE NULL END ) AS total_listing_price FROM tbl_inv_additional_addings AS aa LEFT JOIN tbl_additional_details AS ad ON aa.AU_Id = ad.AU_Id WHERE SP_Id = '".$sp_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT SUM( CASE WHEN ad.AU_Type IN (1, 3) THEN ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2) WHEN ad.AU_Type = 2 THEN ROUND(AD_Unit_Price * 100 / 118, 2) ELSE NULL END ) AS total_listing_price FROM tbl_inv_additional_addings AS aa LEFT JOIN tbl_additional_details AS ad ON aa.AU_Id = ad.AU_Id WHERE SP_Id = '" . $sp_id . "' AND AD_Quantity > 0")->row();
	}
	public function total_additional_char($cus_id)
	{
		//echo("SELECT SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS total_amount FROM tbl_inv_additional_add WHERE CU_Id = '".$cus_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS total_amount FROM tbl_inv_additional_add WHERE CU_Id = '" . $cus_id . "' AND AD_Quantity > 0")->row();
	}
	public function total_additional_dh($cus_id)
	{
		//echo("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS list FROM tbl_inv_additional_add WHERE AU_Id = 11 and  CU_Id = '".$cus_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS listt FROM tbl_inv_additional_add WHERE AU_Id = 15 and  CU_Id = '" . $cus_id . "' AND AD_Quantity > 0")->row();
	}
	public function total_additional_ch($cus_id)
	{
		//echo("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS list FROM tbl_inv_additional_add WHERE AU_Id = 11 and  CU_Id = '".$cus_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS listt FROM tbl_inv_additional_add WHERE AU_Id = 14 and  CU_Id = '" . $cus_id . "' AND AD_Quantity > 0")->row();
	}
	public function total_additional_th($cus_id)
	{
		//echo("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS list FROM tbl_inv_additional_add WHERE AU_Id = 11 and  CU_Id = '".$cus_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS listt FROM tbl_inv_additional_add WHERE AU_Id = 13 and  CU_Id = '" . $cus_id . "' AND AD_Quantity > 0")->row();
	}
	public function total_additional_hh($cus_id)
	{
		//echo("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price * 100 / 118, 2)) AS list FROM tbl_inv_additional_add WHERE AU_Id = 11 and  CU_Id = '".$cus_id."' AND AD_Quantity > 0");exit();
		return $this->db->query("SELECT *,SUM(ROUND(AD_Quantity * AD_Unit_Price , 2)) AS listt FROM tbl_inv_additional_add WHERE AU_Id = 12 and  CU_Id = '" . $cus_id . "' AND AD_Quantity > 0")->row();
	}
	...
	{
		return $this->db->query("select * from tbl_inv_sale_products where AG_Id = '" . $ag_id . "' order by SP_Id DESC LIMIT 0,1")->row();
	}
	public function get_pi_code($cus_id)
	{
		return $this->db->query("select * from tbl_inv_sale_products where CU_Id = '" . $cus_id . "' group by CU_Id")->row();
	}
