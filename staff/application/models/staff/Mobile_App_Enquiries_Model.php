<?php
class Mobile_App_Enquiries_Model extends CI_Model{
	protected $table_name;
    public function __construct()
    {
        parent::__construct();
    }
	public function get_mobile_enquiries($tablearr){
		$view = "SELECT PD.*, SL.*, CU.*, US.* FROM tbl_product AS PD LEFT JOIN tbl_sale_products AS SL ON PD.ID = SL.PD_Id LEFT JOIN tbl_customer AS CU ON SL.CU_Id = CU.CU_Id LEFT JOIN user AS US ON SL.US_Id = US.US_Id WHERE SL.PD_Status != 6 AND US.Usertype = 5 AND SL.CU_Id = CU.CU_Id";
		...
		if($q->result()){
				...
				$slno++;
				$status = array("1"=>"Order Taken","2"=>"Accepted","3"=>"Approved","4"=>"Rejected","5"=>"Completed","8"=>"Cancelled");
				$detail = '<a href="mobile_app_enquiries/detailview/' . $row->CU_Id . '"><i class="fa fa fa-eye"></i></a> <a href="mobile_app_enquiries/mobileEnquary/' . $row->CU_Id . '"><i class="fa fa-mobile"></i></a>';
				$action = '<a href="mobile_app_enquiries/performa_invoice/'.$row->CU_Id.'"><i class="mdi mdi-file-document"></i></a>
