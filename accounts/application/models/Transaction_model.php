...
/*	function updateTransactions($transactionId,$tblTransDataSet,$tblTransDtlDataSet)
	...
	function workOrderList()
	{
		...
		//	$this->db->join('tbl_product','tbl_sale_products.PD_Id=tbl_product.ID');
			$this->db->join('tbl_customer','tbl_customer.CU_Id=tbl_sale_products.CU_Id');
			$this->db->where('tbl_sale_products.PD_Status',5);
			$this->db->group_by('tbl_sale_products.PD_Order_No');
			$query = $this->db->get();
			return $query->result();
	}
	function invoiceInfo($orderNo)
	{
			$this->db->select('PD_Order_No,PD_Order_Date,PD_Delivery_Date,CU_Name,CU_Phone,CU_Address');
			$this->db->from('tbl_sale_products');
			$this->db->join('tbl_customer','tbl_customer.CU_Id=tbl_sale_products.CU_Id');
			$this->db->where('tbl_sale_products.PD_Order_No',$orderNo);
			$this->db->where('tbl_sale_products.PD_Status',5);
			$this->db->limit(1);
			$query = $this->db->get();
			$row=$query->result();
	...
	function invoiceDetails($orderNo)
	{
			$this->db->from('tbl_sale_products');
			$this->db->join('tbl_product','tbl_sale_products.PD_Id=tbl_product.ID');
			$this->db->join('tbl_product_units','tbl_sale_products.PD_Unit=tbl_product_units.UN_Id');
			$this->db->where('tbl_sale_products.PD_Order_No',$orderNo);
			$this->db->where('tbl_sale_products.PD_Status',5);
